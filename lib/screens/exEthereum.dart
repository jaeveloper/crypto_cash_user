...
class ExEthereum extends StatefulWidget {
  final String currentUserId;
  final String userId;

  const ExEthereum({this.currentUserId, this.userId});
  @override
  _ExEthereumState createState() => _ExEthereumState();
}

class _ExEthereumState extends State<ExEthereum> {
  ...
  _setEthereum(BuildContext context) async {
    if (_ethereumformKey.currentState.validate() && !_isLoading) {
      ...
      if (num.parse(olduser.ethereumAmt) > num.parse(ethereumVal)) {
        User setolduser = User(
          id: olduser.id,
          ethereumAmt: (num.parse(olduser.ethereumAmt) - num.parse(ethereumVal))
              .toStringAsFixed(4),
        );
        ...
        User setnewuser = User(
          id: newuser.id,
          ethereumAmt: (num.parse(newuser.ethereumAmt) + num.parse(ethereumVal))
              .toStringAsFixed(4),
        );
        DatabaseService.updateEthereumCoin(setnewuser);
        Scaffold.of(context)
            .showSnackBar(SnackBar(content: Text("Ethereum Sent")));
        TextEditingController().clear();
      } else {
        FocusScope.of(context).unfocus();
        Scaffold.of(context)
