...
class ExStellar extends StatefulWidget {
  final String currentUserId;
  final String userId;

  const ExStellar({this.currentUserId, this.userId});
  @override
  _ExStellarState createState() => _ExStellarState();
}

class _ExStellarState extends State<ExStellar> {
  ...
  _setStellar(BuildContext context) async {
    if (_stellarformKey.currentState.validate() && !_isLoading) {
      ...
      User olduser = await DatabaseService.getUserWithId(widget.currentUserId);
      if (num.parse(olduser.stellarAmt) > num.parse(stellarVal)) {
        User setolduser = User(
          id: olduser.id,
          stellarAmt: (num.parse(olduser.stellarAmt) - num.parse(stellarVal))
              .toStringAsFixed(4),
        );
        ...
        User setnewuser = User(
          id: newuser.id,
          stellarAmt: (num.parse(newuser.stellarAmt) + num.parse(stellarVal))
              .toStringAsFixed(4),
        );
        DatabaseService.updateStellarCoin(setnewuser);
        Scaffold.of(context)
            .showSnackBar(SnackBar(content: Text("Stellar Sent")));
        TextEditingController().clear();
      } else {
        FocusScope.of(context).unfocus();
        Scaffold.of(context)
