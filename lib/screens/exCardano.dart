...
class ExCardano extends StatefulWidget {
  final String currentUserId;
  final String userId;

  const ExCardano({this.currentUserId, this.userId});
  @override
  _ExCardanoState createState() => _ExCardanoState();
}

class _ExCardanoState extends State<ExCardano> {
  ...
  _setCardano(BuildContext context) async {
    if (_cardanoformKey.currentState.validate() && !_isLoading) {
      ...
      User olduser = await DatabaseService.getUserWithId(widget.currentUserId);
      if (num.parse(olduser.cardanoAmt) > num.parse(cardanoVal)) {
        User setolduser = User(
          id: olduser.id,
          cardanoAmt: (num.parse(olduser.cardanoAmt) - num.parse(cardanoVal))
              .toStringAsFixed(4),
        );
        ...
        User setnewuser = User(
          id: newuser.id,
          cardanoAmt: (num.parse(newuser.cardanoAmt) + num.parse(cardanoVal))
              .toStringAsFixed(4),
        );
        DatabaseService.updateCardanoCoin(setnewuser);
        Scaffold.of(context)
            .showSnackBar(SnackBar(content: Text("Cardano Sent")));
        TextEditingController().clear();
      } else {
        FocusScope.of(context).unfocus();
        Scaffold.of(context)
