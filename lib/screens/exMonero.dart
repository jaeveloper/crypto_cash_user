...
class ExMonero extends StatefulWidget {
  final String currentUserId;
  final String userId;

  const ExMonero({this.currentUserId, this.userId});
  @override
  _ExMoneroState createState() => _ExMoneroState();
}

class _ExMoneroState extends State<ExMonero> {
  ...
  _setMonero(BuildContext context) async {
    if (_moneroformKey.currentState.validate() && !_isLoading) {
      ...
      User olduser = await DatabaseService.getUserWithId(widget.currentUserId);
      if (num.parse(olduser.moneroAmt) > num.parse(moneroVal)) {
        User setolduser = User(
          id: olduser.id,
          moneroAmt: (num.parse(olduser.moneroAmt) - num.parse(moneroVal))
              .toStringAsFixed(4),
        );
        ...
        User setnewuser = User(
          id: newuser.id,
          moneroAmt: (num.parse(newuser.moneroAmt) + num.parse(moneroVal))
              .toStringAsFixed(4),
        );
        DatabaseService.updateMoneroCoin(setnewuser);
        Scaffold.of(context)
            .showSnackBar(SnackBar(content: Text("Monero Sent")));
        TextEditingController().clear();
      } else {
        FocusScope.of(context).unfocus();
        Scaffold.of(context)
