...
class ExLitecoin extends StatefulWidget {
  final String currentUserId;
  final String userId;

  const ExLitecoin({this.currentUserId, this.userId});
  @override
  _ExLitecoinState createState() => _ExLitecoinState();
}

class _ExLitecoinState extends State<ExLitecoin> {
  ...
  _setLitecoin(BuildContext context) async {
    if (_litecoinformKey.currentState.validate() && !_isLoading) {
      ...
      if (num.parse(olduser.litecoinAmt) > num.parse(litecoinVal)) {
        User setolduser = User(
          id: olduser.id,
          litecoinAmt: (num.parse(olduser.litecoinAmt) - num.parse(litecoinVal))
              .toStringAsFixed(4),
        );
        ...
        User setnewuser = User(
          id: newuser.id,
          litecoinAmt: (num.parse(newuser.litecoinAmt) + num.parse(litecoinVal))
              .toStringAsFixed(4),
        );
        DatabaseService.updateLitecoinCoin(setnewuser);
        Scaffold.of(context)
            .showSnackBar(SnackBar(content: Text("Litecoin Sent")));
        TextEditingController().clear();
      } else {
        FocusScope.of(context).unfocus();
        Scaffold.of(context)
