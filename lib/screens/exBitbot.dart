...
class ExBitbot extends StatefulWidget {
  final String currentUserId;
  final String userId;

  const ExBitbot({this.currentUserId, this.userId});
  @override
  _ExBitbotState createState() => _ExBitbotState();
}

class _ExBitbotState extends State<ExBitbot> {
  ...
  _setLitecoin() async {
    if (_litecoinformKey.currentState.validate() && !_isLoading) {
      ...
      User olduser = await DatabaseService.getUserWithId(widget.currentUserId);
      if (num.parse(olduser.litecoinAmt) > num.parse(litecoinVal)) {
        User setolduser = User(
          id: olduser.id,
  ...
  _setBitbot(BuildContext context) async {
    if (_bitbotformKey.currentState.validate() && !_isLoading) {
      ...
      User olduser = await DatabaseService.getUserWithId(widget.currentUserId);
      if (num.parse(olduser.bitbotAmt) > num.parse(bitbotVal)) {
        User setolduser = User(
          id: olduser.id,
          bitbotAmt: (num.parse(olduser.bitbotAmt) - num.parse(bitbotVal))
              .toStringAsFixed(4),
        );
        ...
        User setnewuser = User(
          id: newuser.id,
          bitbotAmt: (num.parse(newuser.bitbotAmt) + num.parse(bitbotVal))
              .toStringAsFixed(4),
        );
        DatabaseService.updateBitbotCoin(setnewuser);
        Scaffold.of(context)
            .showSnackBar(SnackBar(content: Text("Bitbot Sent")));
        TextEditingController().clear();
      } else {
        FocusScope.of(context).unfocus();
        Scaffold.of(context)
