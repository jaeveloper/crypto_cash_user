...
class ExBynase extends StatefulWidget {
  final String currentUserId;
  final String userId;

  const ExBynase({this.currentUserId, this.userId});
  @override
  _ExBynaseState createState() => _ExBynaseState();
}

class _ExBynaseState extends State<ExBynase> {
  ...
  _setBynase(BuildContext context) async {
    if (_bynaseformKey.currentState.validate() && !_isLoading) {
      ...
      if (num.parse(olduser.bynaseAmt) > num.parse(bynaseVal)) {
        User setolduser = User(
          id: olduser.id,
          bynaseAmt: (num.parse(olduser.bynaseAmt) - num.parse(bynaseVal))
              .toStringAsFixed(4),
        );
        ...
        User setnewuser = User(
          id: newuser.id,
          bynaseAmt: (num.parse(newuser.bynaseAmt) + num.parse(bynaseVal))
              .toStringAsFixed(4),
        );
        DatabaseService.updateBynaseCoin(setnewuser);
        Scaffold.of(context)
            .showSnackBar(SnackBar(content: Text("Bynase Sent")));
