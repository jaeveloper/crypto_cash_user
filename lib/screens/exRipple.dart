...
class ExRipple extends StatefulWidget {
  final String currentUserId;
  final String userId;

  const ExRipple({this.currentUserId, this.userId});
  @override
  _ExRippleState createState() => _ExRippleState();
}

class _ExRippleState extends State<ExRipple> {
  ...
  _setEthereum() async {
    if (_ethereumformKey.currentState.validate() && !_isLoading) {
      ...
      User olduser = await DatabaseService.getUserWithId(widget.currentUserId);
      if (num.parse(olduser.ethereumAmt) > num.parse(ethereumVal)) {
        User setolduser = User(
          id: olduser.id,
          ethereumAmt: (int.parse(olduser.ethereumAmt) - num.parse(ethereumVal))
              .toString(),
        );
  ...
  _setRipple(BuildContext context) async {
    if (_rippleformKey.currentState.validate() && !_isLoading) {
      ...
      User olduser = await DatabaseService.getUserWithId(widget.currentUserId);
      if (num.parse(olduser.rippleAmt) > num.parse(rippleVal)) {
        User setolduser = User(
          id: olduser.id,
          rippleAmt: (num.parse(olduser.rippleAmt) - num.parse(rippleVal))
              .toStringAsFixed(4),
        );
        ...
        User setnewuser = User(
          id: newuser.id,
          rippleAmt: (num.parse(newuser.rippleAmt) + num.parse(rippleVal))
              .toStringAsFixed(4),
        );
        DatabaseService.updateRippleCoin(setnewuser);
        Scaffold.of(context)
            .showSnackBar(SnackBar(content: Text("Ripple Sent")));
        TextEditingController().clear();
      } else {
        FocusScope.of(context).unfocus();
        Scaffold.of(context)
  ...
  _setZain() async {
    if (_zainformKey.currentState.validate() && !_isLoading) {
      ...
      User olduser = await DatabaseService.getUserWithId(widget.currentUserId);
      if (int.parse(olduser.zainAmt) > int.parse(zainVal)) {
        User setolduser = User(
          id: olduser.id,
          zainAmt: (int.parse(olduser.zainAmt) - int.parse(zainVal)).toString(),
        );
        DatabaseService.updateZainCoin(setolduser);
        User newuser = await DatabaseService.getUserWithId(recieverAddress);
        User setnewuser = User(
          id: newuser.id,
          zainAmt: (int.parse(newuser.zainAmt) + int.parse(zainVal)).toString(),
        );
