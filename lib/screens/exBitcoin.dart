...
class ExBitcoin extends StatefulWidget {
  final String currentUserId;
  final String userId;

  const ExBitcoin({this.currentUserId, this.userId});
  @override
  _ExBitcoinState createState() => _ExBitcoinState();
}

class _ExBitcoinState extends State<ExBitcoin> {
  ...
  _setBitcoin(BuildContext context) async {
    if (_bitcoinformKey.currentState.validate() && !_isLoading) {
      ...
      User olduser = await DatabaseService.getUserWithId(widget.currentUserId);
      if (num.parse(olduser.bitcoinAmt) > num.parse(bitcoinVal)) {
        User setolduser = User(
          id: olduser.id,
          bitcoinAmt: (num.parse(olduser.bitcoinAmt) - num.parse(bitcoinVal))
              .toStringAsFixed(4),
        );
        ...
        User setnewuser = User(
          id: newuser.id,
          bitcoinAmt: (num.parse(newuser.bitcoinAmt) + num.parse(bitcoinVal))
              .toStringAsFixed(4),
        );
        DatabaseService.updateBitcoinCoin(setnewuser);
        Scaffold.of(context)
            .showSnackBar(SnackBar(content: Text("Bitcoin Sent")));
        TextEditingController().clear();
      } else {
        FocusScope.of(context).unfocus();
        Scaffold.of(context)
